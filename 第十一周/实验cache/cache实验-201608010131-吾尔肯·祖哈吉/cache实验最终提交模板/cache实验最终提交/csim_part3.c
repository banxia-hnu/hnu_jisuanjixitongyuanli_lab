...
#definecsim_get_value(number, mask, offset) (((number) & (mask)) >> (offset))
...
CSim_Cache * csim_construct_cache(intset_number,intline_number,intblock_offset);
void csim_deconstruct_cache(CSim_Cache ** pcache);
/* cache simulation function */
CSim_Cache_Result csim_parse_trace_file(CSim_Cache * cache, FILE * file_pointer,charverbose_flag);
